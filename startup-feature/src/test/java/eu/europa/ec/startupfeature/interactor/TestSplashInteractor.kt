...
import eu.europa.ec.authenticationlogic.usecase.ResolveLocalAuthRouteUseCaseImpl
...
class TestSplashInteractor {
    ...
    fun before() {
        ...
        runBlocking { whenever(recoveryCheckpointController.getCheckpoint()).thenReturn(RecoveryCheckpoint.NONE) }
    ...
    fun `Given session is Authenticated and profile complete and wallet active and PIN exists and unlocked, When determineStartupState is called, Then returns Ready`() =
        coroutineRule.runTest {
            ...
            whenever(quickPinInteractor.getLocalUnlockStatus()).thenReturn(LocalUnlockStatus.ReadyForPin)
    ...
    fun `Given PIN not set, When determineStartupState is called, Then returns PinNotSet`() =
        coroutineRule.runTest {
            ...
            whenever(isProfileCompletedUseCase.invoke()).thenReturn(true)
            whenever(isWalletActivatedUseCase.invoke()).thenReturn(WalletActivationStatus.Activated)
            whenever(quickPinInteractor.getLocalUnlockStatus()).thenReturn(LocalUnlockStatus.NotProvisioned)
            whenever(prefsController.safeBool(LocalAuthKeys.ENROLLMENT_REQUIRED, false)).thenReturn(true)
    ...
    @Test
    fun `Given wallet is activated and local auth material is missing unexpectedly, When determineStartupState is called, Then returns PinRecoveryRequired`() =
        coroutineRule.runTest {
            setupAuthenticatedSession()
            whenever(isProfileCompletedUseCase.invoke()).thenReturn(true)
            whenever(isWalletActivatedUseCase.invoke()).thenReturn(WalletActivationStatus.Activated)
            whenever(quickPinInteractor.getLocalUnlockStatus()).thenReturn(LocalUnlockStatus.NotProvisioned)
            whenever(prefsController.safeBool(LocalAuthKeys.ENROLLMENT_REQUIRED, false)).thenReturn(false)

            val result = interactor.determineStartupState()

            assertEquals(StartupState.PinRecoveryRequired, result)
            verify(quickPinInteractor).reportTamperDetected(
                listOf("unexpected_missing_auth_material")
            )
        }

    @Test
    fun `Given wallet recovery checkpoint is pending, When determineStartupState is called, Then local unlock flow is bypassed and wallet setup is required`() =
        coroutineRule.runTest {
            setupAuthenticatedSession()
            whenever(isProfileCompletedUseCase.invoke()).thenReturn(true)
            whenever(isWalletActivatedUseCase.invoke()).thenReturn(WalletActivationStatus.Activated)
            ...
            assertTrue(result is StartupState.WalletNotActivated)
        }

    @Test
    fun `Given local auth material is tampered, When determineStartupState is called, Then returns SecurityError`() =
        coroutineRule.runTest {
            setupAuthenticatedSession()
            whenever(isProfileCompletedUseCase.invoke()).thenReturn(true)
            whenever(isWalletActivatedUseCase.invoke()).thenReturn(WalletActivationStatus.Activated)
            whenever(quickPinInteractor.getLocalUnlockStatus()).thenReturn(LocalUnlockStatus.TamperDetected)

            val result = interactor.determineStartupState()

            assertTrue(result is StartupState.SecurityError)
            verify(quickPinInteractor).reportTamperDetected(
                listOf("local_auth_integrity_failure")
            )
        }
