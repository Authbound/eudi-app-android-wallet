...
class AuthboundPidIntroViewModel(
    ...
    private fun handleCandourSdkResult(status: String) {
        ...
        when (status.uppercase(Locale.ROOT)) {
            ...
            "FAILED" -> navigateToResult(AuthboundPidResultType.FAILED)
            "CANCELLED" -> navigateToResult(AuthboundPidResultType.CANCELED)
            "UNSUPPORTED_NFC" -> navigateToResult(AuthboundPidResultType.CANCELLED_UNSUPPORTED_DEVICE)
            "NO_INTERNET" -> navigateToResult(AuthboundPidResultType.NO_INTERNET)
            "UNAVAILABLE" -> navigateToResult(AuthboundPidResultType.UNAVAILABLE)
            "TIMEOUT" -> navigateToResult(AuthboundPidResultType.TIMEOUT)
            "ERROR",
            "NO_VALID_SAVED_DATA" -> navigateToResult(AuthboundPidResultType.UNKNOWN)
            else -> navigateToResult(AuthboundPidResultType.UNKNOWN)
        }
    }
