...
class TestActionsViewModel {
    ...
    @Test
    fun `Given an opaque wallet refresh, When received, Then actions are fetched through the authenticated interactor`(): Unit =
        coroutineRule.runTest {
            ...
            notifications.emit(
                WalletNotification(
                    ...
                    data = mapOf("type" to "wallet_refresh")
                )
            )
            testScope.advanceUntilIdle()
    ...
    private fun createViewModel(): ActionsViewModel = ActionsViewModel(
